<template>
    ...
    <swiper v-show="isSwiperReady" :slides-per-view="1" :breakpoints="{
        ...
        <swiper-slide v-for="podcast in filteredPodcasts" :key="podcast.data.title" class="px-6">
            <a :href="`/resources/podcasts/${podcast.slug}`"
                ...
                <h3 class="font-bold xs:col-start-2 xs:row-start-1">{{ podcast.data.title }}</h3>
                <p class="text-sm line-clamp-2 xs:line-clamp-6 mb-2 xs:mb-0 xs:col-start-2 xs:row-start-2"
                    :title="podcast.data.shortDescription">{{
                        podcast.data.shortDescription }}</p>

                <ImageSkeleton
                    class="self-end place-self-center aspect-square w-48 xs:w-36 xs:col-start-1 xs:row-start-1 xs:row-span-2 bg-gray-300">
                    <img :src="podcast.data.image" alt="podcast" :style="{
                        '--view-transition-name': podcast.slug ? podcast.slug : podcast.data.title.replaceAll(' ', ''),
...
const filteredPodcasts = computed(() => {
    ...
    return podcasts.filter((podcast) => podcast.data.theme === $selectedPodcast.value);
});
